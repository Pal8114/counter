...
import 'package:counter/src/screens/follow_details.dart';
...
class _BookstoreNavigatorState extends State<BookstoreNavigator> {
  ...
  Widget build(BuildContext context) {
    ...
    if (pathTemplate == '/author/:authorId') {
      String pid = routeState.route.parameters['authorId']!;
      selectedAuthor = Author(0, pid);
          ...
          FadeTransitionPage<void>(
            ...
            child: SignInScreen(
              onSignIn: (credentials) async {
                whoami = (await initCanister(canisterId, url))!;
                //print(whoami);
                //initCanister(canisterId, url);

                var signedIn =
                    await authState.signIn(credentials.cid, credentials.curl);
                if (signedIn) {
